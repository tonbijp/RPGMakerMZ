...
( () => {
  ...
  // Game_Map.tileUnit は HalfMove.js 対応
  ...
  Scene_Map.prototype.onMapLoaded = function () {
    ...
    const fixedMapArgs = PluginManagerEx.findMetaValue(
      $dataMap,
      TAG_FLEXED_MAP
    );
  ...
  Sprite_Actor.prototype.setActorHome = function (index) {
    ...
    const x =
      PARTY_X +
      (Graphics.boxWidth - DEFAULT_SCREEN_WIDTH) +
      index * HORIZONTAL_DIFF;
    const y =
      PARTY_Y +
      (Graphics.boxHeight - DEFAULT_SCREEN_HEIGHT) +
      index * VERTICAL_DIFF;
    this.setHome(x, y);
  };
  ...
  Game_Picture.prototype.show = function (
    ...
    x = Math.floor(x / TF_zoomScale);
    y = Math.floor(y / TF_zoomScale);
    scaleX = Math.floor(scaleX / TF_zoomScale);
    scaleY = Math.floor(scaleY / TF_zoomScale);
  ...
  function stringToVariableId(name) {
    ...
    i = parseInt(name, 10);
    if (isNaN(i))
      throw Error(`${PLUGIN_NAME}: I can't find the variable '${name}'`);
    return i;
  }
  ...
  function stringToEventId(value) {
    ...
    switch (lowValue) {
      ...
      case EVENT_FOLLOWER1:
        return FOLLOWER_OFFSET - 1;
      case EVENT_FOLLOWER2:
        return FOLLOWER_OFFSET - 2;
      case VEHICLE_BOAT:
        return VEHICLE_OFFSET;
      case VEHICLE_SHIP:
        return VEHICLE_OFFSET - 1;
      case VEHICLE_AIRSHIP:
        return VEHICLE_OFFSET - 2;
    }

    const e = $dataMap.events.find((e) => e && e.name === value);
    if (e === undefined)
      throw Error(`${PLUGIN_NAME}: I can't find the event '${value}'`);
    return e.id;
  }
